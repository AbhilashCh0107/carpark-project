...
public class ParkingPinDetailActivity extends AppCompatActivity {
    ...
    private BuySlotDialog slotBuyDialog;
    private Runnable funcBuyPackege;
    private Runnable funcPayLater;
    Calendar calendar ;
    int year ;
    int month;
    int date ;
...
//        SlotRecycleeView.setAdapter(slotsAdapter);
        ...
        DatePickButtonToday.setOnClickListener(v -> {
            DatePickerDialog.OnDateSetListener listener=new DatePickerDialog.OnDateSetListener() {
                ...
                public void onDateSet(DatePicker view, int yearSelected, int monthOfYear, int dayOfMonth)
                {
                     ...
                     month =monthOfYear;
                     date = dayOfMonth;
                     DatePickButtonToday.setText("Selected Date: "+date+"/"+month+"/"+year);
                    picBtnPressed(year,month,date);

                }};

            DatePickerDialog dpDialog=new DatePickerDialog(this, listener, year, month, date);
        ...
        booking_final_Btn.setOnClickListener(v -> {
            funcBuyPackege = () -> {
                parkingBooking.setPayLater(false);
                finishActivityAfterBooking(parkingBookingTransation,parkingBooking);
            };

            funcPayLater = () -> {
                parkingBooking.setPayLater(true);
                parkingBookingTransation.setAmount("0");
                finishActivityAfterBooking(parkingBookingTransation,parkingBooking);
            };

            slotBuyDialog = new BuySlotDialog(parkingBookingTransation,parkingBooking,this,funcBuyPackege,funcPayLater,R.style.PackageBuyAnimation_SmileWindow,false);
    ...
    private void finishActivityAfterBooking(Transaction parkingBookingTransation, ParkingBooking parkingBooking) {
