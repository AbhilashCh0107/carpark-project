...
public class NetworkServices {
    static private final DatabaseReference REF = FirebaseDatabase.getInstance().getReference();
    ...
    public static class ProfileData {
        ...
        public static void updateData(String Name, String email, String carNumber, Context con) {

            final Map<String, Object> UserdataMap = new HashMap<>();
            UserdataMap.put("Name", Name);
            UserdataMap.put("Email", email.trim());
            UserdataMap.put("CarNumber", carNumber.trim());

            mProfileReference.updateChildren(UserdataMap).addOnCompleteListener(v -> {
                if (v.isSuccessful()) {
                    Toasty.success(con, "Sucessfully Updated Profile").show();
                    final Map<String, Object> mCarDetailMap = new HashMap<>();
                    mCarDetailMap.put(carNumber.trim(), FirebaseAuth.getInstance().getCurrentUser().getUid());
                    DatabaseReference mCarDetail = REF.child("CarDetail");
                    mCarDetail.updateChildren(mCarDetailMap).addOnCompleteListener(h -> {
                        if (h.isSuccessful()) {
//                            Toasty.success(con,"Sucessfully Updated Profile").show();
                        } else {
//                            Toasty.error(con,"Please try again!").show();
                        }
                    });
                } else {
                    Toasty.error(con, "Please try again!").show();
        ...
        public static void setData(Object name_et, Object email_et, Object car_number_et, Object mobile_et) {
            ...
            mProfileReference.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if (userProfile != null) {
                        ...
                        if (car_number_et != null) {
                            if (car_number_et instanceof EditText) {
                                EditText carNumber = (EditText) car_number_et;
                                carNumber.setText(userProfile.CarNumber);
                            }
                            if (car_number_et instanceof TextView) {
                                TextView carNumber = (TextView) car_number_et;
                                carNumber.setText(userProfile.CarNumber);
                            }
                        }
        ...
        public static void getProfileDataById(String number_plate, Object name_et, Object email_et, Object car_number_et, Object mobile_et) {
            DatabaseReference cardetail = REF.child("CarDetail").child(number_plate.replace("-", "").toLowerCase());

            cardetail.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    String userId = dataSnapshot.getValue(String.class);
                    if (userId != null) {
                        DatabaseReference userRef = REF.child("Users").child(userId).child("Profile");
                        userRef.addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
        ...
        public static void getProfileDataById(Context context, String id) {
            DatabaseReference userRef = REF.child("Users").child(id).child("Profile");
            ...
            userRef.addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    UserProfile user = dataSnapshot.getValue(UserProfile.class);
                    if (user != null) {
                        SharedPreferences sh = context.getSharedPreferences("myinfo", MODE_PRIVATE);
                        SharedPreferences.Editor edit = sh.edit();
                        edit.putString(Theparker.SP_User_id, id);
                        edit.putString(Theparker.SP_Mobileno, user.Mobile_no);
                        edit.putString(Theparker.SP_Name, user.Name);
                        edit.putString(Theparker.Role, user.Role);
                        edit.apply();
                    }
                }
...
//                    Toast.makeText(getApplicationContext(),""+dataSnapshot,Toast.LENGTH_LONG).show();
    ...
    public static class ParkingPin {
        ...
        static DatabaseReference mBookingRef = REF.child("Users").child(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid()).child("MyParkingBookings");
...
//                    LocationPin pin = new LocationPin(by,description,price,type,numberofspot,pinkey,features,pinloc,address,mobile,area);
        ...
        public static void getBookingList(List<ParkingBooking> parkingBookings, BookingAdapter bookingAdapter) {
            mBookingRef.addChildEventListener(new ChildEventListener() {
                @Override
                public void onChildAdded(@NonNull DataSnapshot dataSnapshot, @Nullable String s) {
//                    Map<value, Object> pinsSnapshot = (Map<value, Object>) dataSnapshot.getValue();
                    String value = dataSnapshot.getValue(String.class);
                    if (value != null) {
                        DatabaseReference userRef = REF.child("GlobalBookings").child(value);
                        userRef.addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                ParkingBooking parkingBooking = dataSnapshot.getValue(ParkingBooking.class);
                                if (parkingBooking != null) {
                                    parkingBooking.setId(value);
//                                    parkingBookings.add(parkingBooking);
                                    bookingAdapter.updateItem(parkingBooking);
                                }
                            }
...
//
        ...
        public static void payForParkingSlot(Transaction transaction, ParkingBooking parkingBooking, boolean payLater, boolean isRefund) {
            ...
            if (isRefund){
                int newbalance = Integer.parseInt(userProfile.Balance) + (Integer.parseInt(transaction.getAmount())/2);
                Map<String, Object> balanceUpdate = new HashMap<>();
                balanceUpdate.put("Balance", "" + newbalance);
                ProfileData.mProfileReference.updateChildren(balanceUpdate);
            }
            ...
            Map<String, Object> balanceUpdate = new HashMap<>();
            balanceUpdate.put("payLater", false);
            balanceUpdate.put("completed", true);
            Booking.mGlobalBookings.child(parkingBooking.getId()).updateChildren(balanceUpdate);
    ...
    public static class Booking {
        ...
        public static void getSlotData(LocationPin locationPin, int year, int monthOfYear, int dayOfMonth, List<Map<String, Object>> slotsDataList, SlotsAdapter slotsAdapter) {
            ...
            timeSlot.add("7 am to 9 am");
            timeSlot.add("9 am to 11 am");
            timeSlot.add("11 am to 1 pm");
            timeSlot.add("1 pm to 3 pm");
            timeSlot.add("3 pm to 5 pm");
            timeSlot.add("5 pm to 7 pm");
            timeSlot.add("7 pm to 9 pm");
        ...
        private static void bookParkingSpot(ParkingBooking parkingBooking) {
            ...
            bookingMap.put(ParkingBookingConstants.slot, parkingBooking.getSlot());
            bookingMap.put(ParkingBookingConstants.slotNumber, parkingBooking.getSlotNumber());
            bookingMap.put(ParkingBookingConstants.price, parkingBooking.getPrice());
            bookingMap.put(ParkingBookingConstants.timestamp, ServerValue.TIMESTAMP);
            bookingMap.put(ParkingBookingConstants.date, ParkingPinDetailActivity.dayOfMonth + "/" + ParkingPinDetailActivity.monthOfYear + "/" + ParkingPinDetailActivity.Year);
            bookingMap.put(ParkingBookingConstants.transactionId, parkingBooking.getTransactionId());
            bookingMap.put(ParkingBookingConstants.payLater, parkingBooking.isPayLater());
            bookingMap.put(ParkingBookingConstants.completed, false);
