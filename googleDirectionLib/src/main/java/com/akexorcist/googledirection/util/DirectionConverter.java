...
public class DirectionConverter {
    ...
    public static PolylineOptions createPolyline(Context context, ArrayList<LatLng> locationList, int width, int color) {
        PatternItem DOT = new Dot();PatternItem GAP = new Gap(20f);
        List<PatternItem> PATTERN_POLYGON_ALPHA = Arrays.asList(GAP, DOT);
        PolylineOptions rectLine = new PolylineOptions().width(dpToPx(context, width)).color(color).geodesic(true).pattern(PATTERN_POLYGON_ALPHA);
